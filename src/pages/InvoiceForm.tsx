...
export const InvoiceForm: React.FC = () => {
  ...
  const buildInvoiceWithItems = (invoiceData: any, itemsData: any[]) => {
    return {
      ...invoiceData,
  ...
  const generateMissingPdf = useCallback(
    async (
      invoiceDataRaw: any,
      invoiceWithItems: any,
      clientData: any,
      profileData: any
    ) => {
      if (!id) return null;
      ...
      try {
        ...
        const pdfInvoiceData = {
          document_number: invoiceDataRaw.document_no,
          date: invoiceDataRaw.date,
          client_name: invoiceDataRaw.client_name || clientData?.name || '',
          client_address: clientData?.address || '',
          client_tax_number: clientData?.tax_number || '',
          client_number: invoiceDataRaw.client_number || clientData?.client_number || '',
          currency: invoiceDataRaw.currency,
          ...
          vat_enabled: invoiceDataRaw.tax_percent > 0,
          vat_rate: invoiceDataRaw.tax_percent || 0,
          notes: invoiceDataRaw.notes || '',
          service_period_start: invoiceDataRaw.work_period_start,
          service_period_end: invoiceDataRaw.work_period_end,
          object_address: invoiceDataRaw.object_address || '',
          invoice_language: invoiceDataRaw.invoice_language || language,
        };
        ...
        const companyData = {
          ...
          company_tax_number: profileData?.tax_number || '',
          company_bank: profileData?.bank_name || '',
          company_iban: profileData?.iban || '',
          company_bic: profileData?.bic || '',
        };

        let pdfBlob: Blob;
        try {
          pdfBlob = await generateInvoicePDFBlob(
            pdfInvoiceData,
            companyData,
            profileData?.logo_url || undefined
          );
        } catch (error) {
          console.error('PDF with logo failed, retrying without logo:', error);
          pdfBlob = await generateInvoicePDFBlob(pdfInvoiceData, companyData, undefined);
        }
        ...
        const fileName = `${user.id}/${id}-invoice-${Date.now()}.pdf`;
        ...
        console.error('Failed to generate missing PDF:', error);
        return null;
      } finally {
  ...
  const fetchInvoice = useCallback(async () => {
    try {
      ...
      const invoiceWithItems = buildInvoiceWithItems(invoiceData, itemsData || []);
      ...
      if (!invoiceData.pdf_url) {
        await generateMissingPdf(invoiceData, invoiceWithItems, clientData, profileData);
      }
    } catch (error) {
      console.error(error);
  ...
  }, [generateMissingPdf, id, navigate, showError, t]);
          ...
          {isGeneratingPdf ? (
            ...
            <div className="bg-white/10 backdrop-blur-xl border border-white/10 rounded-2xl p-6 text-center">
              ...
              <p className="text-white/60 text-sm mb-4">
                {t('pdfNotAvailable') || 'PDF не вдалося створити'}
              </p>

              <button
                type="button"
                onClick={() => fetchInvoice()}
                className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-white border border-white/10"
              >
                {t('retry') || 'Спробувати ще раз'}
              </button>
            </div>
          )}
        </div>
